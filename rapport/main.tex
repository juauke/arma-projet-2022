...
\tf{ldi.png}{Front montant lors de l'instruction \texttt{ldi}.}{ldi-write.png}{Front descendant lors de l'instruction \texttt{ldi}.}
...
Ce bit de contrôle vaut 1 si et seulement si l'instruction courante est un saut conditionnel c'est-à-dire que \framebox[2cm][c]{op $= 110$} et que la condition associée est vérifiée.
...
\clearpage
        ...
        En plus de l'ajout de la question précédente, on a ajouté un bit de contrôle \textit{do\_jmp\_cnd}, l'arrivée du bit \textit{do\_jcc} comme bit de contrôle au compteur de programme.
...
\hcf{jmp_calc.png}{Calcul de l'adresse de saut.}
    ...
    On calcule l'adresse de saut à l'aide de l'adresse courante et de l'\texttt{imm5} que l'on remplace par son opposé que l'on soustrait lorsqu'il est négatif.
...
\tf{mux-mem_access.png}{Multiplexeur qui permet de décaler négativement l'adresse en mémoire.}{mux-read_mem.png}{Multiplexeur qui permet de lire en mémoire lors d'un \texttt{ld}.}
...
\clearpage
        ...
        \begin{itemize}
            ...
            Donc : $b-a \geq 0$ sans avoir dépassé la capacité de l'ALU c'est-à-dire que \framebox[1.5cm][c]{$a \leq b$}.
